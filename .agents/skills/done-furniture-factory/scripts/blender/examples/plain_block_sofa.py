"""Plain block-arm 3-seat sofa, r001 - Route C (procedural static Blender), PARAMETRIC in width/depth/height/colour.

Run:  blender -b -P build_sofa.py -- --width 2.10 --depth 0.92 --height 0.82 --color "#d9cbb3" --out exports/x.glb
Blender axes: X width, Y depth (-Y = front), Z up. glTF export makes front +Z, origin centred X/Y, base at 0.
Design vocabulary borrowed (inspiration only): plain 3-seater, square vertical block arms, short cylindrical wood legs, two seat + two back cushions.
Changed: own proportions, crowned arms, fine piped seams, tapered turned-style legs with slight splay.
Fabric = Poly Haven Hessian 380 (CC0), luminance-normalised, colour = baseColorFactor multiply (weave never changes).
"""
import sys, os
...
D = opt("--depth", 0.92)
H = opt("--height", 0.82)
COLOR = opt("--color", "#d8cab2")
OUT = os.path.abspath(opt("--out", os.path.join(HERE, "exports", "plain-block-sofa_r001.glb")))
random.seed(23)

# ---- derived construction (metres) -------------------------------------------------------------------------------
LEG_H = 0.15
FZ = 0.26                       # top of the fabric base frame
SEAT_T = 0.19
SEAT_TOP = FZ + SEAT_T - 0.02
AT = 0.20                       # arm thickness
AH = min(0.62, H * 0.75)        # arm top height
FLARE = 0.0                    # arm top leans outward by this much
ARM_Z0 = LEG_H + 0.005
BACK_T = 0.13
...
def leg(name, x, y, mat, top=0.040, bot=0.030, splay=0.02, sx=1, sy=1):
    bm = bmesh.new()
...
# back frame (rear)
back = rounded_box("backframe", INNER_W + 0.06, BACK_T, AH - 0.06 - LEG_H, 0, 0.035, 1, fab, FAB_TILE, seg=8)
...
for i, s in enumerate((-1, 1)):
    c = rounded_box(f"backc{i}", cw - 0.004, BC_T, bc_h, 0, 0.06, 3, fab, FAB_TILE, seg=8)
    puff(c, 0.0, 0.04, 0.004, front=0.05)
    bz0 = min(v.co.z for v in c.data.vertices)
    ps = [piping(f"backpipe{i}{k}", (cw - 0.004) / 2 - 0.004, BC_T / 2 - 0.004, bz0 + z, 0.06, 0.0045, fab, FAB_TILE)
