    ...
    <div role="group" aria-label="Colour and finish" style={{ display: "flex", alignItems: "center", gap: 6 }}>
      {group.map((v) => (
        <PdSwatch
          ...
          // The colour name is optional in the catalog, so it cannot be relied
          // on alone as the swatch's name; the product name always is.
          tip={v.colors?.[0]?.name ? `${v.name} · ${v.colors[0].name}` : v.name}
          size={16}
