...
// Real-model-only (IKEA + BlenderKit + Poly Haven + Sketchfab + Poly Pizza) —
// see IKEA_ASSETS / BLENDERKIT_ASSETS / POLYHAVEN_ASSETS / SKETCHFAB_ASSETS /
// POLYPIZZA_ASSETS below. Geometry is normalized at load time (scaled so the model's plan
...
export const CATALOG_BY_ID: ReadonlyMap<string, FurnitureAsset> = new Map(
  [...CATALOG, ...IKEA_ASSETS, ...BLENDERKIT_ASSETS, ...POLYHAVEN_ASSETS, ...SKETCHFAB_ASSETS, ...POLYPIZZA_ASSETS].map(
    (a) => [a.assetId, a],
  ),
);
