...
// taxonomy — free-form per source (e.g. BlenderKit's tags). No source knows
...
// the normalized English item-type added by each source's enrich/normalize
// step), and are applied in catalog.ts at build time. They deliberately live
// in code rather than being baked into data/*.catalog.json: those files are
// generated by each source's build-catalog.ts, so a hand-edit there is lost
...
export const RETAG_RULES: RetagRule[] = [
  // Outdoor: some sources suffix the type itself (", outdoor" / ", in/outdoor"),
 ...
 * is unchanged for items no rule touches.
 */
