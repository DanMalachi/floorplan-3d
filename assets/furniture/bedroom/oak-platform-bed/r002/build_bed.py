...
OUT = os.path.join(HERE, "exports", "oak-platform-bed_r002.glb")
random.seed(7)
...
# --- pillows: sealed two-layer bag, inflated by cloth pressure, dropped against the headboard
def make_pillow(name, cx, cy, cz, tilt, yaw):
    A, B, K, N = 0.34, 0.235, 3.0, 2.6
    bm = bmesh.new()
    bmesh.ops.create_grid(bm, x_segments=46, y_segments=34, size=1.0)
    for v in bm.verts:
        x, y = v.co.x, v.co.y
        m = max(abs(x), abs(y)); t = (abs(x) ** K + abs(y) ** K) ** (1 / K)
        k = (m / t) if t > 1e-9 else 0.0
        v.co.x, v.co.y, v.co.z = x * k * A, y * k * B, 0.006 if m < 0.9999 else 0.0
    bmesh.ops.mirror(bm, geom=list(bm.verts) + list(bm.edges) + list(bm.faces), axis="Z", merge_dist=1e-5)
    bmesh.ops.remove_doubles(bm, verts=list(bm.verts), dist=1e-5)
    bmesh.ops.recalc_face_normals(bm, faces=list(bm.faces))
    bm.normal_update()
    fabric_uv(bm, FAB_TILE)
    bm.transform(Matrix.Translation((cx, cy, cz)) @ Matrix.Rotation(yaw, 4, "Z") @ Matrix.Rotation(tilt, 4, "X"))
    ob = new_obj(name, bm)
    ob.data.materials.append(M_WHITE)
    return ob


def pillow_setup(st, co):
    st.quality = 10
    st.mass = 0.05
    ...
    st.uniform_pressure_force = 6.0
    st.use_pressure_volume = True
    st.target_volume = 0.038
...
top = DECK_TOP + MATT_H
pillows = []
for nm, cx, yaw, tilt in (("pillow_L", -0.42, 0.05, math.radians(-14)), ("pillow_R", 0.42, -0.07, math.radians(-14))):
    po = make_pillow(nm, cx, 0.74, top + 0.09, tilt, yaw)
    pillows.append(cloth_bake(po, 70, pillow_setup))
