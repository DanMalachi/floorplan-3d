...
1. **Layer/color/stroke metadata check first** — may be free (paper 5.2 step 1). Check whether `W2`, `W26/27/28`, `W46/47/48` differ from confirmed real walls (`W0`, `W22`, `W23`) by PDF layer name, color, or stroke attribute. If they live on a distinguishable layer/color, the filter needs no thresholds at all.
2. **Periodicity/repetition signature second**, only if metadata doesn't separate them — the observed pattern is exactly paper 5.6 Layer 2's named signature (1-D FFT along the candidate population to catch periodic hatching/mullions/stair-treads), and is what milestone-2-step-2 already predicted would eventually be needed. Deterministic geometry only, no cross-evidence voting (that's P6's).
...
Start the next session with the **layer/color/stroke metadata check** on the identified spurious candidates (`W2`, `W26/27/28`, `W46/47/48` vs. `W0`/`W22`/`W23` on 30x50). Do not touch closure's SPLIT-side bound. Do not re-run or re-derive the pinned transform without first resolving anchor IDs via `wall_parent_ids`. Bring a diagnosis to the first STOP, not a fix — same discipline as every prior round this milestone.
