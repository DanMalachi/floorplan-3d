"""READ-ONLY audit (2026-07-26), priority 1 of the next-session spec in
reports/p3a-notch-resolution.md: the 57 `a_genuine_gt_defect_between_rooms`
edges with opening_coverage >= 0.65 that appeared *because* of the
2026-07-22 classify() notch-branch fix (previously mislabeled
'e_opening_doorway_notch' under the old single-condition opening_coverage
>= 0.8 cutoff) and have never been individually audited.

Same method as extraction/synth/qa/audit_notch_blind_spot.py (the original
8-edge audit): render an overlay + compute door witness, perpendicularity
to nearest wall-backed ring neighbor, jamb-length ratio, residual-gap
profile, and whether check_plan currently suppresses the edge (it never
does for these -- gate_flip_check_audited.py already confirmed
currently_suppressed=False for all 57 -- this script re-derives it anyway
as a cross-check, not an assumption).

Enumeration is (post-fix classify()==a_genuine_gt_defect_between_rooms,
opening_coverage>=0.65) MINUS whatever's already in
audited_notch_ground_truth.json -- should reproduce exactly the 57 keys
gate_flip_check_audited.py's UNAUDITED list names, which is itself checked
below (count + key-set match) before rendering anything.

Does NOT modify check_plan, classify(), or any constant.
"""
...
from extraction.synth.qa.measure_clean_at_source import (
    OPENING_COVERAGE_THRESHOLD as CHECK_PLAN_OPENING_FLOOR,
...
def main():
    ...
    audited_keys = {(e["plan_id"], e["room"], e["edge_index"]) for e in gt["audited_edges"]}

    risk_edges = []  # (raw_plan, e) for every post-fix a-classed edge >=0.65, not yet audited
    ...
    print(f"Unaudited risk-band edges found: {len(risk_edges)} (expect 57, matching "
          f"gate_flip_check_audited.py's UNAUDITED list).")
    if len(risk_edges) != 57:
        print("*** COUNT MISMATCH vs the 57 named in reports/p3a-notch-resolution.md -- "
              "STOP, report this discrepancy before proceeding. ***")
    ...
    for idx, (raw_plan, e) in enumerate(risk_edges):
        ...
        out_path = REPORTS_DIR / f"notch57_audit_{idx:02d}_{tag}_{raw_plan.get('id')}_{e['room']}_e{i}.png"
        ...
        row = dict(
            ...
            opening_coverage=round(e["opening_coverage"], 3), door_witness=has_witness,
            witness_type=witness_type,
            cos_to_neighbor=round(cos_to_neighbor, 3) if cos_to_neighbor is not None else None,
    ...
    out_json = REPORTS_DIR.parent / "qa" / "unaudited_57_signals.json"
    out_json.write_text(json.dumps(rows, indent=2))
    print(f"\nWrote signal table to {out_json}")
