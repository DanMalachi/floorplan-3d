...
| `public/furniture/factory/tex/*` (texture tiles for the live parametric factory ports, `src/parametric/factory/`) | Resized WebP copies of the SAME maps the factory GLBs above already embed — Poly Haven Rough Linen / Oak Veneer 01 / Curly Teddy Natural / Hessian 380 / Velour Velvet (CC0) plus the own-work leather grain and Astra bed maps; upholstery colour maps are the same luminance-normalised grey derivatives. Per-file sources: `public/furniture/factory/tex/manifest.json` | CC0-1.0 / owner-controlled own work, as the rows above | ✅ ship + redistribute + commercial use (no new source introduced) |
