...
|---|---|---|---|
| `public/materials/doors/veneer/*/{color,normal,roughness}.webp` (15 door wood species) | Poly Haven veneer scans, per-species source URL in `public/materials/doors/veneer/manifest.json` (oak_veneer_03/04/05, silver_oak_veneer_01, washed_grey_oak_veneer, black_oak_veneer, american_walnut_veneer, european_walnut_veneer_04, walnut_veneer_02, ash_veneer, white_maple_veneer, cherry_veneer, teak_veneer, sapele_veneer_02, flamed_black_veneer). Licence checked at https://polyhaven.com/license 2026-09-25. Colour maps re-graded to the finished species colour by `scripts/doors/bake-veneers.mjs` | CC0-1.0 | ✅ ship + redistribute + commercial use. Door micro-textures (paint orange peel, powder coat, laminate stipple, brushing, glass flutes) are generated at runtime by `src/doors/materials.ts`: own work, no source image |
...
| `public/furniture/factory/upholstered-queen-bed.glb` (+ `.png` thumbnail) | Done-original "Astra benchmark" upholstered queen bed, authored 2026-09-16/18 by the Astra (OpenAI Codex) agent for Dan (`scripts/curated/build-bed-benchmark.py`, branch `codex/furniture-expansion`), sha256 `e7d3d236…e93d`. Geometry AND all three texture sets (natural-oak, oatmeal-upholstery, ivory-washed-linen) authored in-house; no external model or material files, no branded design copied | Geometry + textures: original; its own LICENSE.txt states a CC0 1.0 dedication (written by the authoring agent; Dan has not separately recorded it). No third-party rights involved | ✅ ship + redistribute + commercial use. Visually approved by Dan 2026-09-18 in `/dev/furniture`; shipped 2026-09-21 (13 MB, 143k triangles, heaviest asset in the catalog) |
...
| `public/furniture/factory/tex/*` (texture tiles for the live parametric factory ports, `src/parametric/factory/`) | Resized WebP copies of the SAME maps the factory GLBs above already embed — Poly Haven Rough Linen / Oak Veneer 01 / Curly Teddy Natural / Hessian 380 / Velour Velvet / Terlenka (CC0; Terlenka + its off-white/white cotton derivatives are the oak platform bed's, already embedded in its GLB) plus the own-work leather grain and Astra bed maps; upholstery colour maps are the same luminance-normalised grey derivatives. Per-file sources: `public/furniture/factory/tex/manifest.json` | CC0-1.0 / owner-controlled own work, as the rows above | ✅ ship + redistribute + commercial use (no new source introduced) |
