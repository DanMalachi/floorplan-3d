...
in-room Share popover + Save-a-copy. **S4 shipped + verified LIVE on production**: Liveblocks env
on Vercel, OG link-preview card, grant expiry (30-day stateless revoke), Deployment Protection
disabled (production public), deployed. Prod guest test passed: view link joins the host room over
the network ("2 here"), sees the model, no edit UI; /api/share + auth (tampered→403) + OG all 200.
DONE. Future (optional): real model thumbnail in the OG; instant per-link revoke (needs a store).
