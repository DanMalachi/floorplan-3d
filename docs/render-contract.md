...
**Fourth pass: generation defaults, and Studio.** Two more asks, both scoped tight:

- `seedRoomFixtures` (`src/fixtures/seedRoomFixtures.ts`) now bakes `targetLux: FIXTURE_LUX_MAX` (20,000 — the slider's own max, one named constant, not a second copy of the number) and `colorK: GENERATED_FIXTURE_COLOR_K` (4000K neutral) onto every fixture it generates — Dan's explicit ruling for what a freshly-traced room should look like on first view. This is deliberately separate from `DEFAULT_FIXTURE_LUX`/`DEFAULT_FIXTURE_COLOR_K` (3000/2700K, unchanged): a fixture a user drags in from the catalog by hand still starts at the cozier general default; only generation starts maxed. Rooms past `LARGE_ROOM_SPLIT_M2` (40 m² — the same number §10 already flagged as where one fixture starts underlighting a room's far end) now seed more than one lamp, spread along the room's longer axis and validated against the actual polygon (falls back to the pole for any candidate a concave room puts outside itself), capped at 4.

- **Studio ("none" preset) read as "completely dark, needs a light source."** `STUDIO.fillLux` 6,000 → 20,000 (`lightPresets.ts`) — this drives the `hemisphereLight` in `Environment3d.tsx`, the one light in the whole rig with no shadow/occlusion at all, so it's the only thing that reaches an enclosed interior regardless of windows or fixtures, and the old value was ~3x dimmer than a typical outdoor daytime hemisphere with nothing else to fall back on indoors. `STUDIO.keyLux` 20,000 → 40,000 alongside it, mainly for the exterior/facade look (a directional light, blocked by walls same as the sun, so its indoor effect is smaller). **Calibration note, not a full re-verification:** `/calibration`'s "none" cells use these same constants and are now stale — the M1c appendix already states that whole 9-cell set is "PROVISIONAL... NOT frozen" (unlike the M2 formula below), so this isn't reopening a frozen clause, but a recapture is still owed next time someone runs a calibration pass.

**Fifth pass: a real bug (new/regenerated plans got zero lights), Studio pushed much further, and outdoor shadow-side walls.**

- **Bug, not a tuning issue: a freshly generated plan showed no fixtures at all.** Root cause in `preserveSceneEdits` (`src/lib/scene/preserveEdits.ts`): it carried `prev.fixtures` forward onto every regenerate unconditionally, same as furniture. For furniture that's merely cosmetically stale after a big trace correction; for fixtures it was actively broken, because `seedRoomFixtures` only seeds when `fixtures` is `undefined` — a regenerate onto completely unrelated geometry (a new plan, or a different house traced over an old one) left a *defined but geometrically meaningless* fixtures array sitting there, permanently blocking seeding for rooms that in fact had zero relevant lights. Fixed: only fixtures whose `resolveFixtureWorldXY` still lands inside one of the *regenerated* scene's own eligible rooms survive; if none do, `fixtures` is left off the result entirely (not set to `[]`), so `setScene`'s `seedRoomFixtures` treats it exactly like a first-ever generate and seeds the new rooms properly. Covered by `preserveEdits.test.ts` (in-place correction survives, room-renumbering survives, unrelated retrace comes back `undefined`). Accepted narrow tradeoff: a plan where every fixture was deliberately cleared, then given a small in-place trace correction, gets reseeded rather than staying dark — a much smaller problem than a regenerated house silently getting no lights.

- **Studio pushed much further: `fillLux` 20,000 → 80,000, `keyLux` 40,000 → 100,000.** Still "super dark" after the first pass. 80,000 sits close to `REFERENCE_SUN_LUX` (100,000) itself — Studio is no longer trying to be a physically modest fill, it's meant to read as generously lit regardless of scene content. Same calibration caveat as the first pass applies, compounded.

- **Outdoor: walls facing away from the sun read too dark.** New `OUTDOOR_SHADOW_FILL_BOOST = 4` (`lightPresets.ts`), multiplying only the outdoor `hemisphereLight`'s intensity in `Environment3d.tsx`. This renderer has no real bounce/GI — a shadow-side wall gets *only* the hemisphere contribution (direct sun and the measured IBL rig never reach it), so at the physically-measured dome-partition split alone it reads far darker than a real wall would with light bouncing off the ground and its surroundings back onto it. Dan's ruling matches that physical intuition directly ("sunlight still fills the whole environment and reflects by the ground"). Deliberately applied only to the raw hemisphere intensity, not to `IBL_RIG_GAIN`/`envIntensityForSky` (the measured, §7.2.4-audited reflections math) — brightens diffuse wall/floor shading, doesn't touch material reflections.

**Sixth pass: opening a door in walkthrough made the whole app laggy — a real performance bug, unrelated to brightness.** `WalkthroughMode.tsx`'s door-swing animation (`doors.ts`'s `dampOpeningValue`) writes a fresh `Opening` object every animation frame via `updateGesture`, for the ~1-2s a door takes to swing. `WallGroup` (`WallMesh.tsx`) received that wall's openings array (`ops`) as a single `useMemo` dependency covering its ENTIRE body — gap cuts, sill/lintel volumes, baseboard band, and the real `THREE.BufferGeometry` built from them (disposed and recreated on every change, per its own `useEffect` cleanup) — none of which actually reads `swingDeg`/`slide.open` at all, only the door LEAF does (`OpeningPick`, fed separately). So every frame of every door's swing was disposing and rebuilding its entire host wall's real geometry for no reason. Fixed with `wallBodyGeomEqual` + a small per-`WallGroup` cache (`geomOps`): the body's `useMemo` now depends on a stabilized array that only changes reference when a geometry-relevant field actually differs, reusing the previous reference (and therefore skipping the whole rebuild, including geometry disposal) when only swing/slide-open animated. The leaf keeps receiving the live, unstabilized `ops` directly, so it still animates every frame exactly as before — this only removes redundant work the wall body was never supposed to be doing.
